...
U=subset(nodejs, select=c("user_id","seance","part_id"))
...
for(i in 1:(nparts))
{
  ...
  nodejs.Reads[which(nodejs.Reads$part_id==parts[i]),]$Actions_nb =nrow(nodejs[which(nodejs$part_id==parts[i]),])
...
for(i in 1:nchapters){
  children = nodejs.structure[which(nodejs.structure$parent_id==chapters[i]),]$part_id  
  nodejs.Reads[which(nodejs.Reads$part_id==chapters[i]),]$Readers = sum(nodejs.Reads[which(nodejs.Reads$part_id%in%children),]$Readers)
  nodejs.Reads[which(nodejs.Reads$part_id==chapters[i]),]$Actions_nb = sum(nodejs.Reads[which(nodejs.Reads$part_id%in%children),]$Actions_nb)
...
for(i in 1:ntomes){
  ...
  nodejs.Reads[which(nodejs.Reads$part_id==tomes[i]),]$Readers =  sum(nodejs.Reads[which(nodejs.Reads$part_id%in%children),]$Readers)
  nodejs.Reads[which(nodejs.Reads$part_id==tomes[i]),]$Actions_nb = sum(nodejs.Reads[which(nodejs.Reads$part_id%in%children),]$Actions_nb)
